...
export default function userHandlers(io, socket) {
  ...
  console.log(
    "la fonction se rends encores *//////////////////////////////////////"
  );
  socket.on("verification:code", async (userData) => {
    try {
      const { deviceId, code, phoneNumber , phoneHash } = dataParse(userData);
      await verifyUserSMS(socket, code, deviceId, phoneNumber, phoneHash);
    } catch (error) {
      console.error(
  ...
  socket.on("sync_user_contacts", async (data, callback) => {
   await handleSyncContacts(socket, data, callback);
    /* Comment determiner les amis à partir des contacts 
        const friends = await User.find({
    phoneHash: { $in: contacts },          // eux sont dans mes contacts
    contactsHash: { $in: [user.phoneHash] } // et moi suis dans leurs contacts
...
*/
  ...
  socket.on("update-location", async (data) => {
    try {
      ...
      if (!updatedUser.profile.isShyMode) {
        const nearbyRoom = `geo:${Math.floor(coordinates[0])},${Math.floor(
          coordinates[1]
        )}`;
      ...
      socket.emit("error", {
        message: "Erreur de serveur lors de la mise à jour de la localisation",
      });
    }
  });
  ...
  socket.on("find-nearby-users", async (data) => {
    try {
      ...
      const nearbyUsers = await User.find(query)
        .select(
          "userPseudo profile.photo profile.visibility location lastConnection"
        )
        .limit(50);
      ...
      console.error(
        "Erreur lors de la recherche d'utilisateurs à proximité:",
        error
      );
      socket.emit("error", {
        message: "Erreur de serveur lors de la recherche d'utilisateurs",
      });
    }
  });
  ...
  socket.on("profile-viewed", async (data) => {
    try {
      ...
      if (!viewerId || !profileId) {
        return socket.emit("error", {
          message: "Informations de visionnage incomplètes",
        });
      }
      ...
      console.error(
        "Erreur lors de l'enregistrement de la vue du profil:",
        error
      );
      socket.emit("error", {
        message: "Erreur de serveur lors de l'enregistrement de la vue",
      });
    }
  });
  ...
  socket.on("toggle-shy-mode", async (data) => {
    try {
      ...
      socket.emit("error", {
        message: "Erreur de serveur lors de la mise à jour du mode discret",
      });
    }
  });
  ...
  socket.on("change-visibility", async (data) => {
    try {
      ...
      if (!userId || !["public", "private", "friends"].includes(visibility)) {
        return socket.emit("error", {
          message: "Données de visibilité invalides",
        });
      }
      ...
      socket.emit("error", {
        message: "Erreur de serveur lors du changement de visibilité",
      });
    }
  });
...
function joinGeoRooms(socket, coordinates) {
  ...
  const mainGeoRoom = `geo:${Math.floor(coordinates[0])},${Math.floor(
    coordinates[1]
  )}`;
  ...
  const subGeoRoom = `geo:${(Math.floor(coordinates[0] * 10) / 10).toFixed(
    1
  )},${(Math.floor(coordinates[1] * 10) / 10).toFixed(1)}`;
