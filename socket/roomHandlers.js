...
export function setupRoomSocket(io, socket) {
    ...
    socket.on('join:room', async ({ roomId, accessCode, paymentMethod }) => {
        ...
        try {
            ...
            socket.emit('room:joined:error', {
                code: error.message.includes('Insufficient balance') ? 'INSUFFICIENT_BALANCE' :
                    error.message.includes('Invalid access code') ? 'INVALID_ACCESS_CODE' :
                        'JOIN_FAILURE',
    ...
    socket.on('request:room:refund', async ({ roomId }) => {
        ...
        try {
            ...
            // Leave socket room
            socket.leave(roomId);
            const notificationLeave = [...room.members, socket.userData._id.toString()].map(async (memberId) =>
                await createNotification({
                    recipient: memberId,
                    type: 'room_update',
                    content: {
    ...
    socket.on('update:room', async ({ roomId, updates }) => {
        try {
            ...
            updatedRoom.price = socket.userData._id === room.creator ? updatedRoom.price : room.price; // Ensure price is not modified, only the creator can change it
    ...
    socket.on('leave:room', async ({ roomId }) => {
        ...
        try {
            ...
            // Si c'est le dernier membre, on supprime la room
                ...
                room.wallet.balance <= 0 && await Room.findByIdAndDelete(roomId).session(session);
    ...
    socket.on('kick:member', async ({ roomId, userId }) => {
        ...
        try {
            ...
            if (!room.admins.some(admin => admin.toString() === socket.userData._id.toString())) {
                throw new Error('Seuls les administrateurs peuvent expulser des membres');
            }
        ...
        try {
            ...
            const notificationPromises = room.members.map(async (memberId) =>
                await createNotification({
                    ...
                    content: {
                        title: 'Member Banned',
                        message: `${socket.userData.userName}--(KSD:${socket.userData.KSD})  has been banned from  the room "${room.name}". Raison: ${reason || 'Aucune raison spécifiée'}`,
                        roomId: room._id
                    }
                })
            );
    ...
    socket.on('rate-group', async (data) => {
        ...
        try {
            ...
            if (!roomId || rating === undefined || rating < 1 || rating > 5) {
                return socket.emit('error:rating', { message: 'Informations d\'évaluation invalides' });
            }
            ...
            socket.emit('error:rating', { message: 'Erreur serveur lors de l\'évaluation du groupe' });
        }
    });
}